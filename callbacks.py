...
#### Date Picker Callback
@app.callback(
    Output("output-container-date-picker-range-ga-category", "children"),
    [Input("my-date-picker-range-ga-category", "start_date"), Input("my-date-picker-range-ga-category", "end_date")],
)
def update_output(start_date, end_date):
    string_prefix = "You have selected "
    if start_date is not None:
...
# Callback and update first data table
@app.callback(
    Output("datatable-ga-category", "data"),
    [Input("my-date-picker-range-ga-category", "start_date"), Input("my-date-picker-range-ga-category", "end_date")],
)
def update_data_1(start_date, end_date):
...
# Callback and update data table columns
@app.callback(Output("datatable-ga-category", "columns"), [Input("radio-button-ga-category", "value")])
def update_columns(value):
    if value == "Complete":
        column_set = [{"name": i, "id": i, "deletable": True} for i in columns_complete]
    elif value == "Condensed":
...
def update_link(start_date, end_date):
    return "/cc-travel-report/ga-category/urlToDownload?value={}/{}".format(
        dt.strptime(start_date, "%Y-%m-%d").strftime("%Y-%m-%d"), dt.strptime(end_date, "%Y-%m-%d").strftime("%Y-%m-%d")
    )
...
@app.callback(
    ...
    [Input("datatable-ga-category", "selected_rows"), Input("my-date-picker-range-ga-category", "end_date")],
)
def update_ga_category(selected_rows, end_date):
    ...
    for i in selected_rows:
        ...
        # Filter by specific product
    filtered_df = (
        df[(df["GA Category"].isin(travel_product))]
        .groupby(["Year", "Week"])
        .sum()[
            [
                "Spend TY",
...
#### Date Picker Callback
@app.callback(
    Output("output-container-date-picker-range-paid-search", "children"),
    [Input("my-date-picker-range-paid-search", "start_date"), Input("my-date-picker-range-paid-search", "end_date")],
)
def update_output(start_date, end_date):
    string_prefix = "You have selected "
    if start_date is not None:
...
# Callback and update first data table
@app.callback(
    Output("datatable-paid-search", "data"),
    [Input("my-date-picker-range-paid-search", "start_date"), Input("my-date-picker-range-paid-search", "end_date")],
)
def update_data_1(start_date, end_date):
...
# Callback and update data table columns
@app.callback(Output("datatable-paid-search", "columns"), [Input("radio-button-paid-search", "value")])
def update_columns(value):
    if value == "Complete":
        column_set = [{"name": i, "id": i, "deletable": True} for i in columns_complete] + [
...
def update_link(start_date, end_date):
    return "/cc-travel-report/paid-search/urlToDownload?value={}/{}".format(
        dt.strptime(start_date, "%Y-%m-%d").strftime("%Y-%m-%d"), dt.strptime(end_date, "%Y-%m-%d").strftime("%Y-%m-%d")
    )
...
@app.callback(
    ...
    [Input("datatable-paid-search", "selected_rows"), Input("my-date-picker-range-paid-search", "end_date")],
)
def update_paid_search(selected_rows, end_date):
    ...
    for i in selected_rows:
        ...
        # Filter by specific product
    filtered_df = (
        df[(df["Placement type"].isin(travel_product))]
        .groupby(["Year", "Week"])
        .sum()[
            [
                "Spend TY",
...
#### Date Picker Callback
@app.callback(
    Output("output-container-date-picker-range-display", "children"),
    [Input("my-date-picker-range-display", "start_date"), Input("my-date-picker-range-display", "end_date")],
)
...
# Callback and update data table columns
@app.callback(Output("datatable-display", "columns"), [Input("radio-button-display", "value")])
def update_columns(value):
    if value == "Complete":
        column_set = [{"name": i, "id": i, "deletable": True} for i in columns_complete]
    elif value == "Condensed":
...
def update_link(start_date, end_date):
    return "/cc-travel-report/display/urlToDownload?value={}/{}".format(
        dt.strptime(start_date, "%Y-%m-%d").strftime("%Y-%m-%d"), dt.strptime(end_date, "%Y-%m-%d").strftime("%Y-%m-%d")
    )
...
@app.callback(
    ...
    [Input("datatable-display", "selected_rows"), Input("my-date-picker-range-display", "end_date")],
)
...
#### Date Picker Callback
@app.callback(
    Output("output-container-date-picker-range-publishing", "children"),
    [Input("my-date-picker-range-publishing", "start_date"), Input("my-date-picker-range-publishing", "end_date")],
)
...
# Callback and update first data table
@app.callback(
    Output("datatable-publishing", "data"),
    [Input("my-date-picker-range-publishing", "start_date"), Input("my-date-picker-range-publishing", "end_date")],
)
...
# Callback and update data table columns
@app.callback(Output("datatable-publishing", "columns"), [Input("radio-button-publishing", "value")])
def update_columns(value):
    if value == "Complete":
        column_set = [{"name": i, "id": i, "deletable": True} for i in columns_complete]
    elif value == "Condensed":
...
def update_link(start_date, end_date):
    return "/cc-travel-report/publishing/urlToDownload?value={}/{}".format(
        dt.strptime(start_date, "%Y-%m-%d").strftime("%Y-%m-%d"), dt.strptime(end_date, "%Y-%m-%d").strftime("%Y-%m-%d")
    )
...
@app.callback(
    ...
    [Input("datatable-publishing", "selected_rows"), Input("my-date-picker-range-publishing", "end_date")],
)
...
#### Date Picker Callback
@app.callback(
    Output("output-container-date-picker-range-metasearch", "children"),
    [Input("my-date-picker-range-metasearch", "start_date"), Input("my-date-picker-range-metasearch", "end_date")],
)
...
# Callback and update data table columns
@app.callback(Output("datatable-metasearch", "columns"), [Input("radio-button-metasearch", "value")])
def update_columns(value):
    if value == "Complete":
        column_set = [{"name": i, "id": i, "deletable": True} for i in columns_complete]
    elif value == "Condensed":
...
def update_link(start_date, end_date):
    return "/cc-travel-report/metasearch/urlToDownload?value={}/{}".format(
        dt.strptime(start_date, "%Y-%m-%d").strftime("%Y-%m-%d"), dt.strptime(end_date, "%Y-%m-%d").strftime("%Y-%m-%d")
    )
...
@app.callback(
    ...
    [Input("datatable-metasearch", "selected_rows"), Input("my-date-picker-range-metasearch", "end_date")],
)
